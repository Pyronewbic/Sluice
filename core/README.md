...
| file | role |
|---|---|
| `Dockerfile` | `base` (Wolfi + node/git/gh + shell + the firewall stack + non-root `sluice` uid 1000, **no sudo**, no baked key) and `project` (FROM the base; `SLUICE_EXTRA_PKGS` / `SLUICE_EXTRA_NPM` / `SLUICE_SETUP_ROOT_CMDS` / `SLUICE_SETUP_CMDS` / `SLUICE_PREFETCH_*` layers) |
...
| `replay.sh` | baked as `/usr/local/bin/sluice-replay`; a `SLUICE_PIN=1` build replays the exact pinned apk/npm/pip/gem/go/cargo versions and **fails closed** on any drift vs `sluice.lock` |
...
asserted by `test/structure.yaml` (container-structure-test) - the publish gate runs it before
