...
# chown the mounted repo (and any persisted SLUICE_STATE_DIRS / SLUICE_OVERLAY_DIRS volumes) to
# sluice when not already (Linux bind mounts keep the host uid; a fresh named volume mounted over a
# bind path is root-owned and never auto-initialized; no-op at uid 1000 / Docker Desktop).
for d in "${SLUICE_WORKDIR:-}" "${SLUICE_GITDIR:-}" ${SLUICE_STATE_PATHS:-} ${SLUICE_OVERLAY_PATHS:-}; do
  if [ -n "$d" ] && [ -d "$d" ]; then
