...
# Rootless podman maps the host user to container-root, so the entrypoint's chown of the bind-mounted
# repo to the uid-1000 sluice user lands host-side as a SUBUID - the host user then can't write their
# own repo. keep-id:uid=1000,gid=1000 maps the host user straight onto the sluice uid: the mount reads
# as uid 1000 in the box (writable, and the entrypoint chown then no-ops on it, so the host repo stays
# host-owned), while container-internal overlay/state volumes are still chowned as before. Needs podman
...
resolve_podman_userns() {
  ...
  if [ "$maj" -gt 4 ] || { [ "$maj" -eq 4 ] && [ "$min" -ge 3 ]; }; then
    RUNTIME_RUN_OPTS+=("--userns=keep-id:uid=1000,gid=1000")
  else
    echo "${E_YEL:-}[sluice] WARNING:${E_RST:-} rootless podman ${v:-<unknown>} lacks keep-id:uid= (needs >= 4.3); the box will chown the mounted repo to a subuid on the host. Recover write access with: podman unshare chown -R 0:0 \"\$PWD\"" >&2
  fi
}
