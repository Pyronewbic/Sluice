...
npm ls -g --all --json 2>/dev/null | jq -r 'def flat: to_entries[]?|("npm  \(.key) \(.value.version // "")"),(.value.dependencies//{}|flat); (.dependencies//{}|flat)' 2>/dev/null
...
SBOM_ARCH="$(apk --print-arch 2>/dev/null || uname -m)"; export SBOM_ARCH
{
  ...
  npm ls -g --all --json 2>/dev/null | jq -r 'def flat: to_entries[]?|("npm\t\(.key)\t\(.value.version // "")\t"),(.value.dependencies//{}|flat); (.dependencies//{}|flat)' 2>/dev/null | LC_ALL=C sort -u
