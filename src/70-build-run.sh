verify_base() {
  ...
  # Capture cosign's stderr (declare-then-assign so the assignment carries cosign's status, not local's):
  # a signature MISMATCH and a transient outage (offline, Rekor/Fulcio down) both exit nonzero, and
  # swallowing the reason presents an outage as tamper. Surface it on the die so the operator can tell.
  local _verr
  if _verr="$(cosign verify "$img" \
       --certificate-oidc-issuer=https://token.actions.githubusercontent.com \
       --certificate-identity-regexp='^https://github\.com/Pyronewbic/Sluice/\.github/workflows/publish-base\.yml@(refs/tags/v|refs/heads/main$)' 2>&1 >/dev/null)"; then
    ...
    # Fail CLOSED: cosign is present and the official base does not verify. Surface cosign's own words
    # (the tail) so a network/Sigstore outage is distinguishable from a real signature failure, and do
    # NOT advise disabling verification to escape what may be a transient blip.
    [ -n "$_verr" ] && printf '%s\n' "$_verr" | tail -n 5 >&2
    die "cosign verification failed for $img - refusing to build on it. If this is a network or Sigstore outage, retry when online; a genuine signature failure means do NOT build on this image (cosign output above)."
  fi
}
...
build() {
  # Resolve + verify the base BEFORE assembling the context: a resolution or verification die must
  # leave no mktemp dir (holding a copy of the user's config) behind. In pin mode the FROM is the
  # pinned digest, so verify THAT, not the live tag - the pin is already a @sha256 ref, so no pull
  # happens (a signed pin replays air-gapped) and verify_base checks the exact bytes that get built.
  local _pin_active="" _pinbase=""
  if [ "${SLUICE_PIN:-}" = 1 ] && [ "${_SLUICE_PIN_SKIP:-}" != 1 ]; then
    [ -f "$PROJECT_DIR/sluice.pin" ] || die "SLUICE_PIN=1 but no sluice.pin - run 'sluice lock --pin' first"
    _pinbase="$(awk '$1=="base"{print $2; exit}' "$PROJECT_DIR/sluice.pin")"
    case "$_pinbase" in *@sha256:*) ;; *) die "sluice.pin has no @sha256 base digest - re-run 'sluice lock --pin'" ;; esac
    _pin_active=1
  fi
  local _basedig=""
  if [ -n "${SLUICE_BASE_IMAGE:-}" ]; then
    if [ -n "$_pinbase" ]; then
      # Pin mode FROMs the pinned digest; it must be the same repo as the configured base (else re-pin).
      local _pbrepo="${_pinbase%@*}" _birepo="${SLUICE_BASE_IMAGE%@*}"; _birepo="${_birepo%:*}"; _pbrepo="${_pbrepo%:*}"
      [ "$_pbrepo" = "$_birepo" ] || die "config base ($SLUICE_BASE_IMAGE) changed since the pin ($_pinbase) - re-run 'sluice lock --pin'"
      _basedig="$_pinbase"
    else
  ...
  set +f
  # Pinned replay (SLUICE_PIN=1): the Dockerfile always COPY's pin/, so keep a .keep there; drop the
  # real sluice.pin in ONLY when pin mode is active (detected + validated above, before the mktemp).
  mkdir -p "$tmp/pin"; : > "$tmp/pin/.keep"
  [ -n "$_pin_active" ] && cp "$PROJECT_DIR/sluice.pin" "$tmp/pin/sluice.pin"
  ...
  # FROM the verified digest (which IS the pinned digest in pin mode + signed base - resolved above).
  [ -n "$_basedig" ] && args+=(--build-arg "BASE_IMAGE=$_basedig")
  if [ -n "$_pin_active" ]; then
    args+=(--build-arg SLUICE_PIN=1)
    # Local build (no SLUICE_BASE_IMAGE): pin the wolfi base by digest. The signed-base case already
    # set BASE_IMAGE to the pinned digest above, so it is not re-appended here.
    [ -z "${SLUICE_BASE_IMAGE:-}" ] && args+=(--build-arg "WOLFI_BASE=$_pinbase")
  fi
