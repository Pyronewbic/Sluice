...
# ("packages/*/.env" reaches deeper). A matched SYMLINK masks its in-project TARGET (mount the real
# file, never the link); a target outside the mount already dangles in the box. Invalid patterns
# (absolute, ..) are skipped here so doctor still reports the rest; the run path dies on them (mask_validate).
mask_matches() {
  ...
  ( cd "$PROJECT_DIR" 2>/dev/null || exit 0
    ...
    for pat in ${SLUICE_MASK}; do
      ...
      for m in $pat; do
        if [ -L "$m" ]; then
          # Mask the symlink's TARGET (a real file we can safely mount over), not the link. Resolve it
          # physically (macOS: /var is a symlink); a target that escapes the mount dangles in the box,
          # so skip it. Resolution failure also falls back to skip - never worse than the old behavior.
          _lt="$(readlink "$m" 2>/dev/null)" || continue
          [ -n "$_lt" ] || continue
          case "$_lt" in /*) _la="$_lt" ;; *) _la="$(dirname "$m")/$_lt" ;; esac
          _ld="$(cd "$(dirname "$_la")" 2>/dev/null && pwd -P)" || continue
          _la="$_ld/$(basename "$_la")"; _pp="$(pwd -P)"
          case "$_la/" in "$_pp"/*) ;; *) continue ;; esac
          m="${_la#"$_pp"/}"
          if [ -f "$_la" ] || [ -d "$_la" ]; then printf '%s\n' "$m"; fi
          continue
        fi
...
EOF
    if [ -n "$_mtn" ]; then
      ...
      _doc "" "${C_YEL}note${C_RST}: $_ntf masked file(s) are git-tracked - the mask empties the working-tree copy only: in-box git sees them emptied (a commit there would stage the empty file; SLUICE_WORKSPACE=overlay avoids that), but the committed secret stays readable in-box via git history (git show). Remove committed secrets from history."
