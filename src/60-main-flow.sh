...
# Mount-scope guard: the box bind-mounts PROJECT_DIR (the dir of the found sluice.config.sh, walked up
# from $PWD) read-write, with no floor - so launched from $HOME or / it would mount your whole home tree
# / root fs into the sandbox, exposing ~/.ssh, credential stores, everything. Refuse $HOME, /, and any
# ancestor of $HOME unless SLUICE_ALLOW_HOME=1. Pre-build + Docker-free, like the gates below.
guard_home_mount() {
  [ "${SLUICE_ALLOW_HOME:-}" = 1 ] && return 0
  local why=""
  case "$PROJECT_DIR" in /) why="the root filesystem" ;; esac
  if [ -z "$why" ] && [ -n "${HOME:-}" ] && [ "$HOME" != / ]; then
    if [ "$PROJECT_DIR" = "$HOME" ]; then why="your home directory"
    else case "$HOME/" in "$PROJECT_DIR"/*) why="an ancestor of your home directory" ;; esac
    fi
  fi
  [ -n "$why" ] || return 0
  echo "${E_RED}[sluice] refusing:${E_RST} the project dir $(_tilde "$PROJECT_DIR") is $why - the box bind-mounts it" >&2
  echo "         read-write, exposing ~/.ssh, credential stores and the rest of your files to the sandboxed code." >&2
  echo "         Run sluice from inside a project directory, or set SLUICE_ALLOW_HOME=1 to override." >&2
  exit 1
}
case "${1:-run-default}" in run-default|run|shell|build|rebuild|diff) guard_home_mount ;; esac
...
validate_allow_ips() {
  ...
  for e in ${SLUICE_ALLOW_IPS:-}; do
    ...
    # Reject a hostname (non IPv4/CIDR): SLUICE_ALLOW_IPS is fixed IPs (THREAT_MODEL). A hostname is
    # resolved in-box by the allowlist-scoped dnsmasq, which sinks a non-allowlisted name to a dead-end -
    # so the rule would target the sink, not the host. A host belongs in SLUICE_ALLOW_DOMAINS.
    case "${e%%:*}" in
      *[!0-9./]*) die "SLUICE_ALLOW_IPS='$e' is not an IPv4 address/CIDR - it takes fixed IPs (e.g. 10.0.0.5:5432); a hostname resolves to the DNS sink in-box. Put a host in SLUICE_ALLOW_DOMAINS instead." ;;
    esac
