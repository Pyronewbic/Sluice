...
egress_rows() {
  _squid_log "$@" | awk -v allow=" $(allowed_domains) " '
    ...
    END {
      # bytes use %.0f, not %d: mawk casts %d to int32 and truncates a byte total past 2^31 to
      # 2147483647 (~2 GB), so a multi-GB transfer both mis-rendered and could slip under a >2 GiB
      # high-volume flag; %.0f formats the double exactly to 2^53.
      for (h in seen) {
        if (h ~ /^example\.(com|net|org)$/) continue;
        if (succ[h] > 0)        printf "reached\t%s\t%d\t%.0f\n", h, succ[h], bytes[h];
        else if (!allowed(h))   printf "blocked\t%s\t%d\t%.0f\n", h, deny[h], bytes[h];
      }
    }'
}
...
egress_tx_by_host() {
  _squid_log | awk '
    ...
    END { for (h in tot) printf "%s\t%.0f\n", h, tot[h] }'   # %.0f: mawk %d truncates bytes past 2^31
}
...
allowips_rows() {
  _root_exec "$container" iptables -nvxL OUTPUT 2>/dev/null | awk '
    $3=="SLUICE-ALLOWIPS" {
      ...
      printf "%s\t%d\t%.0f\n", entry, $1, $2;   # bytes as %.0f: mawk %d truncates past 2^31
    }'
}
...
fw_dropped() {
  _root_exec "$container" iptables -nvxL OUTPUT 2>/dev/null | awk '
    /^Chain OUTPUT / {
      ...
      printf "%d\t%.0f\n", p, b; exit;   # bytes as %.0f: mawk %d truncates past 2^31
    }'
}
...
# Trailing `| awk` dropped: this is called once per record on the chain walk, so its fork showed up in
# the profile. Shell-strip the filename field instead; output is byte-identical (first field + newline).
# MUST return the hasher's status: the old body ended in a pipeline, so pipefail propagated a broken or
# missing hasher and config_hash died. A body ending in printf always exits 0, which would let
# config_hash return EMPTY and make every `[ "$label" = "$(config_hash)" ]` freshness gate compare
# "" = "" - forever fresh, so a box with a stale firewall/seccomp would never rebuild. Fail closed.
_sha256() {
  local _h _rc
  if command -v sha256sum >/dev/null 2>&1; then _h="$(sha256sum 2>/dev/null)"; else _h="$(shasum -a 256 2>/dev/null)"; fi
  _rc=$?; [ "$_rc" = 0 ] || return "$_rc"
  printf '%s\n' "${_h%% *}"
}
