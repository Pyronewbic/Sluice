...
laundering_host() {
  ...
  case "$1" in
    *s3.amazonaws.com|*.s3.*.amazonaws.com|storage.googleapis.com|*.blob.core.windows.net|*.r2.cloudflarestorage.com|*.digitaloceanspaces.com) return 0 ;;
    ...
    ampcode.com|dashscope.aliyuncs.com|dashscope-intl.aliyuncs.com|api2.cursor.sh|api5.cursor.sh|*.api5.cursor.sh|catwalk.charm.land) return 0 ;;  # agent-provider model/stream hosts shipped presets allow (POST-capable)
  esac
  return 1
}
