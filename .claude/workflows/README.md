...
| `attack-changes` | Adversarially attacks the branch diff: scopes it into areas, one attacker per area hunting a paste-able break, then an independent skeptic re-runs each claim and drops what is pre-existing on the base. Run it BEFORE the PR on anything touching a security path - a green suite only covers the path you happened to exercise. Pass `args.base` (default `main`) and optionally `args.focus`. |
| `demo-beats` | Designs the VHS tape beats for what has shipped: inventories user-visible capabilities since a tag, scores each on security story / recordability / coverage gap, designs a beat for the winners, then a skeptic tries to make each recording pass for the wrong reason. Also calls keep/cut/redo on the existing GIFs against a byte budget. Output only. Pass `args.since` (a tag, default the latest) and optionally `args.ref` (default `main`) / `args.include_uncommitted`. |
